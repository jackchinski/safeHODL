...
const useMultiBaas = () => {
  ...
  const [recentLockBoxes, setRecentLockBoxes] = useState<string[]>([]);
  ...
  const getRecentLockBoxes = useCallback(async (): Promise<string[] | null> => {
    try {
      ...
      return response.data.result.map((e) => {
        ...
        const { data } = JSON.parse(e.event.rawFields) as {
          data: `0x${string}`;
        };
  ...
  const getRecentOpens = useCallback(async (): Promise<string[] | null> => {
    try {

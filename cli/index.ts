...
const contract_address_polygon_sepolia =
  process.env.CONTRACT_ADDRESS_POLYGON || "";
const contract_address_unichain_sepolia =
  process.env.CONTRACT_ADDRESS_UNICHAIN || "";
const contract_address_mantle_sepolia =
  process.env.CONTRACT_ADDRESS_MANTLE || "";
...
figlet("SafeHODL", async function (err, data) {
  ...
  const actionToDo = readlineSync.question(
    log(chalk.blue("Select action to do: (1 - Lock, 2 - Unlock)"))
  );
  ...
  const selectedChain = readlineSync.question(
    log(
      chalk.blue(
        "Select chain: (1 - Polygon Sepolia, 2 - Mantle Sepolia, 3 - Unichain Sepolia)"
      )
    )
  );
  ...
  async function callContract(safeHODLContract, actionToDo) {
    ...
    if (actionToDo == 1) {
      ...
      // Lock
      const receiverEmail = readlineSync.question(
        log(chalk.blue("Provide receiver email address:"))
      );
      const password = readlineSync.question(
        log(chalk.blue("Provide unlocking password"))
      );
      const amountToLock = readlineSync.question(
        log(chalk.blue("Provide amount to lock"))
      );

      // Offline calculate the locking hash
      const lockingHash = solidityPackedKeccak256(
        ["string", "string"],
      ...
      try {
        const tx = await safeHODLContract.lock(receiverEmail, lockingHash, {
          value: String(amountToLock),
        });

        spinner.succeed(
          chalk.blue("🐱 Funds locked. TX sent. Hash: " + tx.hash)
        );
      } catch (error) {
        // If there's an error
      ...
      // Unlock
      const receiverEmail = readlineSync.question(
        log(chalk.blue("Provide your email address:"))
      );
      const password = readlineSync.question(
        log(chalk.blue("Provide your password"))
      );
      const receiverAddress = readlineSync.question(
        log(chalk.blue("Provide destination address:"))
      );
      ...
      try {
        ...
        spinner.succeed(
          chalk.blue("🐱 Funds unlocked. TX sent. Hash: " + tx.hash)
        );
      } catch (error) {
        // If there's an error
        spinner.fail("Locking failed. Please try again.");
